...
/**
 * PPD Document Tracking Slip (A4) with Logos + QR (TransmittalMemo-compatible)
 *
 * Requirements:
...
final class PPDTrackingSlip
{
  ...
  public static function generateA4(array $data, string $absOutPath): void
  {
    ...
    $ppdNo   = trim((string)($data['ppd_tracking_no'] ?? ''));
    $subject = trim((string)($data['subject'] ?? ''));

    if ($ppdNo === '' || $subject === '') {
      throw new RuntimeException('PPDTrackingSlip requires ppd_tracking_no and subject.');
    }
    ...
    $pdf->SetX($centerX);
    $pdf->Cell($centerW, 4.8, 'PLANNING AND PROGRAMMING DIVISION', 0, 1, 'C');
    ...
    $txt($x0 + $leftW + 2, $y + 2.2, 'PPD Tracking No.:', '', 8.0);

    $mpwNo = trim((string)($data['mpw_tracking_no'] ?? ''));
    $txt($x0 + 2, $y + 6.8, $mpwNo, 'B', 10.2);
    $txt($x0 + $leftW + 2, $y + 6.8, $ppdNo, 'B', 10.2);
    ...
    $rH = $namesH / 3.0;
    $line($x0, $y + $rH, $x0 + $w0, $y + $rH);
    $line($x0, $y + 2*$rH, $x0 + $w0, $y + 2*$rH);

    $pdf->SetTextColor(90, 90, 90);
    $txt($x0 + 2, $y + 2.0, '(Names / initials - handwriting)', '', 7.2);
    $pdf->SetTextColor(20, 24, 40);
    ...
    $pdf->Cell($w0, 4.5, 'MACABAI M. PANGAMADUN', 0, 1, 'C');

    $pdf->SetFont('Arial', '', 7.4);
    $pdf->SetX($x0);
    $pdf->Cell($w0, 4.2, 'Chief, Planning and Programming Division', 0, 1, 'C');
    ...
    $pdf->SetXY($x0, $tableY + 2.4);
    $pdf->Cell($col1W, 5, 'RECEIVED BY', 0, 0, 'C');
    $pdf->SetXY($x0 + $col1W, $tableY + 2.4);
    ...
    $line($xL2, $tableY + $hdrH, $xL2, $tableY + $tableH);
    $line($xL3, $tableY + $hdrH, $xL3, $tableY + $tableH);
    ...
    $line($xR2, $tableY + $hdrH, $xR2, $tableY + $tableH);
    $line($xR3, $tableY + $hdrH, $xR3, $tableY + $tableH);
    ...
    $pdf->SetXY($xL + 1.5,  $tableY + $hdrH + 1.5); $pdf->Cell($l_c1 - 3, 4, 'Date/Time', 0, 0, 'L');
    $pdf->SetXY($xL2 + 1.5, $tableY + $hdrH + 1.5); $pdf->Cell($l_c2 - 3, 4, 'Signature', 0, 0, 'L');
    $pdf->SetXY($xL3 + 1.5, $tableY + $hdrH + 1.5); $pdf->Cell($l_c3 - 3, 4, 'Name', 0, 0, 'L');

    $pdf->SetXY($xR + 1.5,  $tableY + $hdrH + 1.5); $pdf->Cell($r_c1 - 3, 4, 'Date/Time', 0, 0, 'L');
    $pdf->SetXY($xR2 + 1.5, $tableY + $hdrH + 1.5); $pdf->Cell($r_c2 - 3, 4, 'Actions/Other Instruction/Remarks', 0, 0, 'L');
    $pdf->SetXY($xR3 + 1.5, $tableY + $hdrH + 1.5); $pdf->Cell($r_c3 - 3, 4, 'Deadline', 0, 0, 'L');
