...
if (!$canGenerate) {
  http_response_code(403);
  echo json_encode(["ok" => false, "error" => "Division tracking slip 2nd page can only be generated by the current holder division"]);
  exit;
}

function division_slip_page2_assistant_name_for_principal(mysqli $conn, int $principalUserId): string
{
...
function division_slip_page2_format_received_datetime_input(?string $raw): string
{
  $raw = trim((string)$raw);
  if ($raw === '') {
    return '';
...
function division_slip_page2_resolve_received_defaults(mysqli $conn, int $documentId, int $divisionId, array $identity, array $doc): array
{
  ...
  if ($defaultReceivedBy === '') {
    $creatorAssistant = division_slip_page2_assistant_name_for_principal($conn, $createdByUserId);
    if ($creatorAssistant !== '') {
  ...
  return [
    ...
    'received_at_display' => division_slip_page2_format_received_datetime_input($defaultReceivedAtRaw),
  ];
}
...
$receivedDefaults = division_slip_page2_resolve_received_defaults($conn, $docId, $divisionId, $identity, $doc);
...
$initialReceiveAtRaw = trim((string)($receivedDefaults['received_at_raw'] ?? ''));
...
$storedName = $safeDivision . '_TRACKING_SLIP_PAGE2_' . $safeTracking . '_' . date('Ymd_His') . '.pdf';
$abs = $docDir . '/' . $storedName;
$rel = 'storage/attachments/doc_' . $docId . '/' . $storedName;

$note = division_tracking_page2_attachment_note($divisionCode);
$supersededNote = $note . ':SUPERSEDED';
$hadExistingPage2 = false;
$stmt = $conn->prepare("
  SELECT id
...
$stmt->execute();
$hadExistingPage2 = (bool)$stmt->get_result()->fetch_assoc();
...
$stmt = $conn->prepare("
  ...
  WHERE document_id = ?
    ...
    AND note = ?
");
$stmt->bind_param('sis', $supersededNote, $docId, $note);
$stmt->execute();
...
DivisionTrackingSlip::generateMovementPageA4([
...
$payload = json_encode([
  'kind' => 'division_tracking_slip_page2_generated',
  'attachment_id' => $attId,
  'file' => $orig,
  'division_code' => $divisionCode,
...
$message = $hadExistingPage2
  ? 'Latest division tracking slip 2nd page generated.'
  : 'Division tracking slip 2nd page generated.';
echo json_encode([
  'ok' => true,
  'attachment_id' => $attId,
